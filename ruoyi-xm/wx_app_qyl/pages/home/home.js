...
const { toListThumbnailUrl, toDetailThumbnailUrl } = require('../../utils/image-url')
...
Page({
    ...
    return this.getUserLocation()
      .then((userLocation) => {
        ...
        return Promise.all([
          ...
          const filteredData = this.buildFilteredLists({ currentMerchant, grouponList })
          this.setData({
  ...
  normalizeMerchant(m = {}) {
    ...
    const albumList = Array.isArray(m.albumList)
      ? m.albumList.filter(Boolean).map((image) => toDetailThumbnailUrl(image))
      : []
    const fallbackImage = toDetailThumbnailUrl(m.coverImage || m.avatar || DEFAULT_PRODUCT_IMAGE)
    const carouselImages = albumList.length ? albumList : [fallbackImage].filter(Boolean)
    return {
      ...m,
  ...
  buildEmptyMerchant() {
    return {
      ...
      bookingText: '',
      displayTags: [],
      carouselImages: [DEFAULT_PRODUCT_IMAGE],
      carouselCount: 1
    }
  },

  buildFilteredLists({ currentMerchant = {}, grouponList = [] }) {
    const merchantId = String(currentMerchant.merchantId || currentMerchant.id || '')
    const merchantItems = merchantId
      ? grouponList.filter((item) => String(item.merchantId || '') === merchantId)
      : []
    return {
      displayGrouponList: merchantItems.length ? merchantItems : grouponList
    }
  },

  onCarouselChange(e) {
    this.setData({ currentCarouselIndex: Number(e.detail.current || 0) })
  },

  chooseMerchant() {
    const merchantList = this.data.merchantList || []
    if (merchantList.length <= 1) {
      util.showToast('当前仅有这一家门店')
      return
    }
    wx.showActionSheet({
      itemList: merchantList.map((merchant) => merchant.name).slice(0, 6),
      success: (res) => {
        const currentMerchant = merchantList[res.tapIndex]
        if (!currentMerchant) return
        const filteredData = this.buildFilteredLists({
          currentMerchant,
          grouponList: this.data.grouponList
        })
        this.setData({
          currentMerchant,
          currentCarouselIndex: 0,
          currentLocation: this.formatCurrentDistance(currentMerchant),
          displayGrouponList: filteredData.displayGrouponList
        })
      }
    })
