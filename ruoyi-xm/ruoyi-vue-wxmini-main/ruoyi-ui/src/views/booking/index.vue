<template>
  <div class="app-container">
    <el-card>
      ...
      <el-form :inline="true" :model="queryParams" size="small" class="search-form">
        <el-form-item label="预约单号">
          <el-input v-model="queryParams.bookingNo" placeholder="请输入预约单号" clearable @keyup.enter.native="handleQuery" />
        </el-form-item>
      ...
      <el-table v-loading="loading" :data="bookingList" border>
        <el-table-column label="预约单号" prop="bookingNo" min-width="170" show-overflow-tooltip />
        ...
        <el-table-column label="预约时间" prop="bookingTime" min-width="160" />
    ...
    <el-dialog title="预约详情" :visible.sync="detailOpen" width="620px">
      <el-descriptions v-if="currentBooking" :column="2" border>
        ...
        <el-descriptions-item label="预约时间">{{ currentBooking.bookingTime || '-' }}</el-descriptions-item>
...
export default {
  ...
  methods: {
    ...
    handleConfirm(row) {
      this.confirmAction('确认该预约？', () => confirmBooking(row.id))
    },
    handleComplete(row) {
      this.confirmAction('标记该预约为已完成？', () => completeBooking(row.id))
    },
    handleCancel(row) {
      this.confirmAction('取消该预约？', () => cancelBooking(row.id))
    },
    handleExpire(row) {
      this.confirmAction('将该预约置为已过期？', () => expireBooking(row.id))
    },
