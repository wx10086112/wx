  ...
  <div class="app-container">
    <el-card>
      ...
      <el-form :inline="true" :model="queryParams" size="small" class="search-form">
        <el-form-item label="商家名称">
          <el-input v-model="queryParams.name" placeholder="请输入商家名称" clearable @keyup.enter.native="handleSearch" />
        </el-form-item>
      ...
      <el-table v-loading="loading" :data="tableList" border style="width: 100%">
        ...
        <el-table-column prop="level" label="等级" width="80" align="center">
          <template slot-scope="scope">
            <el-tag :type="levelTagType(scope.row.level)" size="small">{{ scope.row.level }}级</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="products" label="商品数" width="80" align="center" />
        <el-table-column prop="monthlySales" label="月销售额" width="110" align="center">
          <template slot-scope="scope">
            <span>¥{{ scope.row.monthlySales.toLocaleString() }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog title="商家审核" :visible.sync="auditDialogVisible" width="500px" :close-on-click-modal="false">
      <div class="audit-info">
        ...
        <p><strong>等级：</strong>{{ auditRow.level }}级</p>
      </div>
      <el-divider />
...
export default {
  ...
  methods: {
    ...
    handleReset() {
      this.queryParams = { name: '', level: '', status: undefined }
      this.pageNum = 1
      this.fetchData()
    },
