  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" size="small" class="search-form">
      <el-form-item label="名称" prop="name">
        <el-input v-model="queryParams.name" placeholder="请输入分销商名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="区域" prop="regionName">
        <el-input v-model="queryParams.regionName" placeholder="请输入区域" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button v-hasPermi="['mall:distributor:add']" type="primary" icon="el-icon-plus" size="small" @click="handleAdd">新增分销商</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-hasPermi="['mall:distributor:remove']" type="danger" icon="el-icon-delete" size="small" :disabled="multiple" @click="handleDelete">批量删除</el-button>
      </el-col>
    </el-row>
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="分销商名称" prop="name" min-width="120" show-overflow-tooltip />
      ...
      <el-table-column label="关联商家" prop="merchantCount" width="90" align="center">
        <template slot-scope="scope">
          <el-tag :type="scope.row.merchantCount > 0 ? 'success' : 'info'" size="small">{{ scope.row.merchantCount || 0 }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="状态" width="90" align="center">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status === 1 ? 'success' : 'info'" size="small">{{ scope.row.status === 1 ? '正常' : '禁用' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="200" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button v-hasPermi="['mall:distributor:edit']" size="small" type="text" icon="el-icon-edit" @click="handleEdit(scope.row)">编辑</el-button>
          <el-dropdown @command="(cmd) => handleCommand(cmd, scope.row)" style="margin-left: 10px;">
            <el-button size="small" type="text">
              更多<i class="el-icon-arrow-down el-icon--right"></i>
            </el-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="resetPwd" icon="el-icon-key" v-hasPermi="['mall:distributor:edit']">重置密码</el-dropdown-item>
              <el-dropdown-item command="toggleStatus" v-hasPermi="['mall:distributor:edit']">{{ scope.row.status === 1 ? '禁用' : '启用' }}</el-dropdown-item>
              <el-dropdown-item command="switchView" icon="el-icon-view" v-if="isSuperAdmin">切换视角</el-dropdown-item>
              <el-dropdown-item command="delete" icon="el-icon-delete" divided v-hasPermi="['mall:distributor:remove']" style="color: #F56C6C;">删除</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      const countText = ids.length > 1 ? '（共 ' + ids.length + ' 条）' : ''
      this.$confirm('确认删除选中的分销商？' + countText, '警告', { type: 'warning' }).then(() => {
