      ...
      <el-table v-loading="loading" :data="tableList" border>
        <el-table-column label="类型" width="110" align="center">
          <template slot-scope="scope">
            <el-tag :type="scope.row.type === '订单收入' ? 'success' : 'danger'" size="small">
              {{ scope.row.type }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="平台佣金" width="120">
          <template slot-scope="scope">
            <span :class="scope.row.commission >= 0 ? 'text-primary' : 'text-danger'">
              {{ scope.row.commission >= 0 ? '+' : '' }}¥{{ scope.row.commission.toFixed(2) }}
            </span>
          </template>
        </el-table-column>
