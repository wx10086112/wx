...
CALL add_merchant_column_if_missing('map_claim_status', '`map_claim_status` VARCHAR(32) DEFAULT ''NOT_CLAIMED'' COMMENT ''map claim status'' AFTER `receiver_type`');
CALL add_merchant_column_if_missing('map_poi_id', '`map_poi_id` VARCHAR(128) DEFAULT NULL COMMENT ''map poi id'' AFTER `map_claim_status`');
CALL add_merchant_column_if_missing('map_claim_url', '`map_claim_url` VARCHAR(500) DEFAULT NULL COMMENT ''map claim url'' AFTER `map_poi_id`');
CALL add_merchant_column_if_missing('map_claim_time', '`map_claim_time` DATETIME DEFAULT NULL COMMENT ''map claim time'' AFTER `map_claim_url`');
CALL add_merchant_column_if_missing('map_claim_remark', '`map_claim_remark` VARCHAR(500) DEFAULT NULL COMMENT ''map claim remark'' AFTER `map_claim_time`');

CALL add_merchant_column_if_missing('wx_applyment_id', '`wx_applyment_id` VARCHAR(128) DEFAULT NULL COMMENT ''wx applyment id'' AFTER `map_claim_remark`');
CALL add_merchant_column_if_missing('wx_applyment_state', '`wx_applyment_state` VARCHAR(64) DEFAULT ''NOT_SUBMITTED'' COMMENT ''wx applyment state'' AFTER `wx_applyment_id`');
CALL add_merchant_column_if_missing('wx_applyment_reject_reason', '`wx_applyment_reject_reason` VARCHAR(1000) DEFAULT NULL COMMENT ''wx applyment reject reason'' AFTER `wx_applyment_state`');
CALL add_merchant_column_if_missing('wx_applyment_time', '`wx_applyment_time` DATETIME DEFAULT NULL COMMENT ''wx applyment time'' AFTER `wx_applyment_reject_reason`');
CALL add_merchant_column_if_missing('wx_applyment_finish_time', '`wx_applyment_finish_time` DATETIME DEFAULT NULL COMMENT ''wx applyment finish time'' AFTER `wx_applyment_time`');
...
CALL add_merchant_index_if_missing('idx_merchant_map_claim_status', '(`map_claim_status`)');
CALL add_merchant_index_if_missing('idx_merchant_wx_applyment_id', '(`wx_applyment_id`)');
